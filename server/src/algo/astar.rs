...
/// determinations.
pub fn a_star_search(start: i32, end: i32, map: &mut dyn BaseMap, max_step: i32) -> NavigationPath {
    AStar::new(start, end).search(map, max_step)
}
...
impl AStar {
    ...
    fn search(&mut self, map: &dyn BaseMap, max_step: i32) -> NavigationPath {
        let result = NavigationPath::new();
        while !self.open_list.is_empty() && self.step_counter < max_step {
            self.step_counter += 1;
